...
function Login() {
  ...
  const loginHandler = async (data) => {
    ...
    if (!loginResponse.success) {
      setLoading(false);
      if (loginResponse.error.businessErrorDescription) {
    ...
    if (loginResponse.response.tfaEnabled) {
      console.log(loginResponse);
      setTfaType(loginResponse.response.type);
      setUserId(loginResponse.response.id);
      setIsTfa(true);
      return;
    } else {
      toast.warn(
        "Enable 2FA for added security. Go to Settings > Password and security > Two-factor authentication"
      );
      dispatch(
        login({
  ...
  const confirmAuthenticatorCode = async () => {
    const tfaResponse = await authService.authenticatorCodeVerification({
      userId: userId,
      code: otp,
    ...
    if (!tfaResponse.success) {
      if (tfaResponse.error.message === "WRONG OTP" || tfaResponse.error.message.includes("2FA token with user Id")) {
        setTfaError("Incorrect OTP entered previously. Please enter the correct code");
        return;
      }
  ...
  if (isTfa) {
    return (
      <div className="tfa-container">
        <div className="tfa-container-heading">
          <FontAwesomeIcon icon={faLock} />
          {tfaType === "REGISTERED_EMAIL" ? (
            <p>Enter the 6-digit login code sent to your registered email.</p>
          ) : (
            <p>
        ...
        <div className="tfa-container content">
          <input
            type="password"
          ...
          />
          {tfaError?.length > 0 && <span style={{ color: "red"}}>{tfaError}</span>}
          <button onClick={confirmAuthenticatorCode}>Confirm</button>
        </div>
        {tfaType === "REGISTERED_EMAIL" ? (
          <div className="tfa-info">
            If you didn’t receive the verification email, you can{" "}
            <button>resend the code</button>.
          </div>
        ) : (
          <div className="tfa-info">
            If you're unable to receive a login code from an authentication app,
            you can use one of your <button>backup codes</button>.
          </div>
        )}
      </div>
    );
