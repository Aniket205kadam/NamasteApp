...
function Login() {
  ...
  const loginHandler = async (data) => {
    ...
    if (loginResponse.response.tfaEnabled) {
      ...
      toast.warn(
        "Enable 2FA for added security. Go to Settings > Account > Security > Two-Factor Authentication."
      );
      dispatch(
        login({
  ...
  const confirmAuthenticatorCode = async () => {
    ...
    if (!tfaResponse.success) {
      if (tfaResponse.error.message === "WRONG OTP") {
        toast.error("Entered wrong OTP, entered correct one once!");
        return;
      }
    ...
    return (
      <div className="tfa-container">
        <div className="tfa-container-heading">
          ...
          <p>Enter a 6-digit login code generated by an authentication app.</p>
        </div>
